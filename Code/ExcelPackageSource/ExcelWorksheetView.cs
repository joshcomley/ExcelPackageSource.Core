...
namespace OfficeOpenXml
{
	...
	public class ExcelWorksheetView
	{
		...
		/// </summary>
		protected internal XElement SheetViewElement
		{
			get 
			{
				if (_sheetView == null)
				{
					_sheetView = _xlWorksheet.WorksheetXml.XPathSelectElement("//d:sheetView", _xlWorksheet.NameSpaceManager);
				}
				return _sheetView;
			}
		}
		...
		public bool TabSelected
		{
			...
			{
				bool retValue = false;
				var ret = SheetViewElement.Attribute("tabSelected").Value;
		...
		public bool PageLayoutView
		{
			...
			{
				bool retValue = false;
				string ret = SheetViewElement.Attribute("view").Value;
			...
			set
			{
				if (value)
					...
					SheetViewElement.Attribute("view").Remove();
			}
		}
		#endregion
	}
